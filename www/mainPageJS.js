...
async function switchView(view) {
	...
	['day', 'week', 'month'].forEach(v => els[`${v}View`].classList.toggle('hidden', v !== view));
	await loadViewData();
	render();
}
...
function renderDay() {
	const d = currentDate;
	els.dateTitle.textContent = `${DAYS_PL[d.getDay()]}, ${d.getDate()} ${MONTHS_PL[d.getMonth()]} ${d.getFullYear()}`;
...
function renderWeek() {
	...
	for (let i = 0; i < 7; i++) {
		...
		col.className = 'week-day-col' + (key === today ? ' today' : '') + (key === selected ? ' selected' : '');
...
async function addCategory() {
	...
	if (!name) {
		flashError(catEls.catNameInput);
		return;
	}
...
function renderCategorySelector() {
	...
	categories.forEach(cat => {
		...
		chip.addEventListener('click', () => {
			...
			selectedCatIds = active ? selectedCatIds.filter(id => id !== cat.id) : [...selectedCatIds, cat.id];
